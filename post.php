...
if(isset($_POST['add_depreciation_account'])){
    ...
    mysqli_query($mysqli,"INSERT INTO depreciation_accounts SET depreciation_account_name = '$name', depreciation_account_currency_code = '$currency_code', depreciation_account_notes = '$notes'");

    //Logging
    mysqli_query($mysqli,"INSERT INTO logs SET log_type = 'Depreciation_Account', log_action = 'Create', log_description = '$name', log_ip = '$session_ip', log_user_agent = '$session_user_agent', log_user_id = $session_user_id");
...
if(isset($_POST['df'])){
    ...
    $dt = intval($_POST['dt']);
    $mt = intval($_POST['mt']);
    $result = mysqli_query($mysqli, "SELECT * FROM date_configurations WHERE date_configuration_name = 'tax year'");
    $rowcount = mysqli_num_rows($result);
    
    if($rowcount == 0)
    {
    	mysqli_query($mysqli, "INSERT INTO date_configurations (date_configuration_name, date_configuration_start_day, date_configuration_start_month, date_configuration_end_day, date_configuration_end_month) VALUES('tax year', '$df', '$mf', '$dt', '$mt')");
    }
    else
    {
    	mysqli_query($mysqli, "UPDATE date_configurations SET date_configuration_start_day = '$df', date_configuration_start_month = '$mf', date_configuration_end_day = '$dt', date_configuration_end_month = '$mt' WHERE date_configuration_name = 'tax year'");
    }
    
    
    $_SESSION['alert_message'] = "UPDATED!";
...
if(isset($_POST['add_expense'])){
    ...
    mysqli_query($mysqli,"INSERT INTO expenses SET expense_date = '$date', expense_amount = $amount, expense_tax = $expense_tax_paid, expense_tax_id = $expense_tax_id, expense_currency_code = '$session_company_currency', expense_account_id = $account, expense_vendor_id = $vendor, expense_client_id = $client, expense_category_id = $category, expense_description = '$description', expense_reference = '$reference', expense_lifetime = $expense_lifetime, expense_current_value = $amount, expense_depreciation_annually = $expense_depreciation_annually, expense_depreciation_account_id = $expense_depreciation_account");
    ...
    if ($_FILES['file']['tmp_name'] != '') {
	    ...
	    $expense_id_query = mysqli_query($mysqli, "SELECT * FROM expenses 
	    LEFT JOIN vendors ON expense_vendor_id = vendor_id 
	    LEFT JOIN categories ON expense_category_id = category_id
	    WHERE expense_id = $expense_id");
	    $row = mysqli_fetch_array($expense_id_query);
	    $id = $row['expense_id'];
	    $expense_date = $row['expense_date'];
	    $expense_vendor = $row['vendor_name'];
	    $expense_amount = $row['expense_amount'];
	    $expense_category = $row['category_name'];
	    $new_file_name = "ID=" . $id . '_' . $expense_date . '_' . $expense_vendor . '_' . $expense_amount . '_' . $expense_category . '.png';
	    
	    //check double entry
	    $new_file_name_check = $expense_date . $expense_vendor . $expense_amount . $expense_category . '.png';
	    $expense_receipt_query = mysqli_query($mysqli, "SELECT * FROM expenses");
	    
	    while($row = mysqli_fetch_array($expense_receipt_query))
	    {
	    	$exist_expense_id = $row['expense_id'];
	    	$exist_expense_date = strval($row['expense_date']);
	    	$exist_expense_amount = $row['expense_amount'];
	    	
	    	if(strcmp($exist_expense_date, $expense_date) == 0 && $expense_amount == $exist_expense_amount)
	    	{
	    	    $flag_found = true;
	    	    break;
	    	}
	    ...
	    $dest_path = $upload_file_dir . $new_file_name;
	    $sourceImage = imagecreatefromstring(file_get_contents($file_tmp_path));
	    //resize if it is more than 1mb
	    $maxSize = 1000*1000;
		...
		$newWidth = 1000; // Adjust the desired width as needed
		...
		// Create a new blank image with the calculated dimensions
	    	...
	    	// Copy and resize the source image to the new compressed image
	    	imagecopyresized($compressedImage, $sourceImage, 0, 0, 0, 0, $newWidth, $newHeight, $width, $height);
	    	
	    	imagefilter($compressedImage, IMG_FILTER_GRAYSCALE);
	  
	    	// Save the compressed image as a PNG file
        	imagepng($compressedImage, $dest_path, 5);
	    ...
	    else // if the file is already less than 1mb
	    {
	    	//convert to png and save it
	        $image = imagecreatefromstring(file_get_contents($file_tmp_path));
	        imagefilter($image, IMG_FILTER_GRAYSCALE);	    
	    	imagepng($image, $dest_path, 5);
	    	imagedestroy($image);
	    }
	    
	    //move_uploaded_file($file_tmp_path, $dest_path); this code is from original but no need for it anymore
    ...
    if($flag_found)
    {
    	$_SESSION['alert_message'] = "Expense added" . $extended_alert_description . "Potential duplicate at expense ID " . $exist_expense_id;
    }
    else
    {
...
if(isset($_POST['edit_expense'])){
    ...
    if ($_FILES['file']['tmp_name'] != '') {
	    ...
	    $new_file_name = "ID=" . $id . '_' . $expense_date . '_' . $expense_vendor . '_' . $expense_amount . '_' . $expense_category . '.png';
	    $dest_path = $upload_file_dir . $new_file_name;
            //move_uploaded_file($file_tmp_path, $dest_path); this is from the orginal code but no need for it anymore
    ...
    mysqli_query($mysqli,"UPDATE expenses SET expense_date = '$date', expense_amount = $amount, expense_tax = $expense_tax_paid, expense_tax_id = $expense_tax_id, expense_account_id = $account, expense_vendor_id = $vendor, expense_client_id = $client, expense_category_id = $category, expense_description = '$description', expense_reference = '$reference', expense_lifetime = '$expense_lifetime', expense_current_value = '$amount', expense_depreciation_annually = $expense_depreciation_annually, expense_depreciation_account_id = $expense_depreciation_account WHERE expense_id = $expense_id");
...
//this has been replace by script in report_expense_summary.php
/*if(isset($_POST['export_expenses_csv'])){
    //get record from the database;
    $query = $_POST['expenses_sql'];
    $sql = mysqli_query($mysqli, $query);
    $file_description = $_POST['file_name'];
    if(mysqli_num_rows($sql) > 0){
        $delimiter = ",";
        //$filename = "$session_company_name-Expenses-$file_description.csv";
        $filename_by_user = $_POST['file_name'];
	$filename = $filename_by_user.".csv";
        //create a file pointer
        $f = fopen('php://memory', 'w');

        //set column headers
        $fields = array('Date', 'Amount', 'Vendor', 'Description', 'Category', 'Account', 'Tax Paid', 'Tax');
        fputcsv($f, $fields, $delimiter);

        //output each row of the data, format line as csv and write to file pointer
        while($row = mysqli_fetch_assoc($sql)){
            $lineData = array($row['expense_date'], $row['expense_amount'], $row['vendor_name'], $row['expense_description'], $row['category_name'], $row['account_name'], $row['expense_tax'], $row['tax_name']." ".$row['tax_percent']."%");
            fputcsv($f, $lineData, $delimiter);
        }
...
if(isset($_POST['export_income_revenue_report_csv'])){
    $date_from = sanitizeInput($_POST['date_from']);
    $date_to = sanitizeInput($_POST['date_to']);
    if(!empty($date_from) && !empty($date_to)){
        $date_query = "AND DATE(revenue_date) BETWEEN '$date_from' AND '$date_to'";
    ...
    $sql = mysqli_query($mysqli, "SELECT * FROM revenues		    
    ...
    if(mysqli_num_rows($sql) > 0){
        $delimiter = ",";
        $filename = "$session_company_name-Income Report-$file_name_date.csv";

        //create a file pointer
        $f = fopen('php://memory', 'w');

        //set column headers
        $fields = array('Date', 'Category', 'Amount(inc Tax)', 'Net Profit', 'Tax Paid', 'Tax Name');
        fputcsv($f, $fields, $delimiter);
        ...
        while($row = mysqli_fetch_assoc($sql)){
            ...
            $lineData = array($non_american_format_date, $row['category_name'], $row['revenue_amount'], $row['revenue_net_profit'], $row['revenue_tax'], $row['tax_name']." ".$row['tax_percent']."%");
            fputcsv($f, $lineData, $delimiter);
        }
    ...
    //Logging
    mysqli_query($mysqli,"INSERT INTO logs SET log_type = 'revenue_report', log_action = 'Export', log_description = '$session_name exported revenue_report to CSV File', log_ip = '$session_ip', log_user_agent = '$session_user_agent',  log_user_id = $session_user_id");
...
if(isset($_POST['add_revenue'])){
    ...
    $description = sanitizeInput($_POST['description']);
    $reference = sanitizeInput($_POST['reference']);
    $tax_id = intval($_POST['revenue_tax_id']);
    $client = intval($_POST['client']);
    ...
    if($tax_id > 0){
        ...
        $row = mysqli_fetch_array($sql);
        $tax_percent = floatval($row['tax_percent']);
        $tax_amount = $amount / ($tax_percent + 100) * $tax_percent;
    ...
    $net_profit = $amount - $tax_amount;
    mysqli_query($mysqli,"INSERT INTO revenues SET revenue_date = '$date', revenue_amount = $amount, revenue_tax_id = $tax_id, revenue_tax = $tax_amount, revenue_net_profit = $net_profit, revenue_currency_code = '$currency_code', revenue_payment_method = '$payment_method', revenue_reference = '$reference', revenue_description = '$description', revenue_category_id = $category, revenue_account_id = $account, revenue_client_id = $client");
...
if(isset($_POST['edit_revenue'])){
    ...
    mysqli_query($mysqli,"UPDATE revenues SET revenue_date = '$date', revenue_amount = $amount, revenue_tax = $tax_amount, revenue_tax_id = $tax_id, revenue_net_profit = $net_profit, revenue_currency_code = '$currency_code', revenue_payment_method = '$payment_method', revenue_reference = '$reference', revenue_description = '$description', revenue_category_id = $category, revenue_account_id = $account, revenue_client_id = $client WHERE revenue_id = $revenue_id");
