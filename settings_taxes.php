...
?>
    ...
    <br>
    <?php
    	$sql_tax_year = mysqli_query($mysqli, "SELECT * FROM date_configurations WHERE date_configuration_name = 'tax year'");
    	$tax_year_row = mysqli_fetch_assoc($sql_tax_year);
	$df = $tax_year_row['date_configuration_start_day'];
	$mf = $tax_year_row['date_configuration_start_month'];
	$dt = $tax_year_row['date_configuration_end_day']; 
	$mt = $tax_year_row['date_configuration_end_month']; 
    ?>
    <div class="card card-dark">
        <div class="card-header py-2">
            <h3 class="card-title mt-2"><i class="fas fa-fw fa-balance-scale mr-2"></i>Tax Year</h3>
            <div class="card-tools">
                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#editTaxYearModal"><i class=""></i>Edit</button>
            </div>
        </div>
       	<div class="card-body">
            <div class="table-responsive-sm">
                <table class="table table-striped table-borderless table-hover">
                <tr>
		    <th><a class="text-dark">From</a></th>
		    <th><a class="text-dark">To</a></th>
                </tr>
                <tr>
                    <td><a><?php echo $df."/".$mf;?></a></td>
                    <td><a><?php echo $dt."/".$mt;?></a></td>
                </tr>
                </table>
            </div>
        </div>
...
require_once("settings_tax_year_modal.php");
